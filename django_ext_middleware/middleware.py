...
JAEGER_SET_ATTRIBUTES = 'JAEGER_SET_ATTRIBUTES'
...
def _set_django_attributes(span, request):
    ...
    if params is not None:
        span.add_attribute('django.request.params', str(params))
...
class OpencensusMiddleware(MiddlewareMixin):
    ...
    def process_response(self, request, response):
        ...
        try:
            ...
            set_attributes_method = settings.params.get(
                JAEGER_SET_ATTRIBUTES, '')
            try:
                set_attributes = import_from_string(set_attributes_method, JAEGER_SET_ATTRIBUTES)
            except (ImportError, Exception):
                _set_django_attributes(span, request)
            else:
                set_attributes(span, request)
